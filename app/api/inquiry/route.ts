...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, firstName, lastName, company, email, phone, message, terms } = body

    const senderName = name || `${firstName || ''} ${lastName || ''}`.trim() || 'Contact'

    if ((!name && (!firstName || !lastName)) || !email || !message) {
      return NextResponse.json(
    ...
    const fromEmail = process.env.RESEND_FROM_EMAIL || "Healium Website <no-reply@healiumsono.com>"

    const { data, error } = await resend.emails.send({
      from: fromEmail,
      to: [toEmail],
      replyTo: email,
      subject: `New Inquiry / Demo Request from ${senderName}`,
      html: `
<!DOCTYPE html>
<html>
<head>
        ...
        <table role="presentation" width="100%" border="0" cellspacing="0" cellpadding="0" style="max-width: 600px; background-color: #FFFFFF; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #EFEAE0;">
          ...
          <tr>
            <td style="padding: 32px 32px 16px 32px;">
              <span style="display: inline-block; background-color: #F0EBFA; color: #687FE5; font-size: 12px; font-weight: 600; padding: 4px 12px; border-radius: 20px; text-transform: uppercase; letter-spacing: 0.5px;">Website Inquiry</span>
              <h1 style="margin: 12px 0 6px 0; font-size: 24px; font-weight: 700; color: #2D2D2D; letter-spacing: -0.5px;">Inquiry / Demo Request</h1>
              <p style="margin: 0; font-size: 14px; color: #666666;">Submitted via the website contact form.</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 0 32px 24px 32px;">
              <table role="presentation" width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #FBF9FF; border-radius: 12px; border: 1px solid #EAE3F7; padding: 16px 20px;">
                <tr>
                  <td style="padding: 8px 0; font-size: 13px; font-weight: 600; color: #687FE5; width: 35%;">FULL NAME</td>
                  <td style="padding: 8px 0; font-size: 15px; font-weight: 500; color: #2D2D2D;">${senderName}</td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td style="padding: 8px 0; font-size: 15px; font-weight: 500; color: #2D2D2D;">${company}</td>
                </tr>` : ''}
                ${phone ? `
                <tr>
                  <td style="padding: 8px 0; font-size: 13px; font-weight: 600; color: #687FE5;">PHONE NUMBER</td>
                  <td style="padding: 8px 0; font-size: 15px; font-weight: 500; color: #2D2D2D;"><a href="tel:${phone}" style="color: #2D2D2D; text-decoration: none;">${phone}</a></td>
                </tr>` : ''}
              </table>
            </td>
          </tr>
          ...
          <tr>
            <td align="center" style="padding: 0 32px 32px 32px;">
              <a href="mailto:${email}?subject=Re:%20Inquiry%20-%20Healium%20Sono" style="display: inline-block; background-color: #C8B5E8; color: #2D2D2D; font-size: 15px; font-weight: 600; padding: 14px 36px; border-radius: 30px; text-decoration: none;">Reply to Lead</a>
            </td>
          </tr>
          ...
          <tr>
            <td style="background-color: #F4F1EA; padding: 20px 32px; text-align: center; border-top: 1px solid #EFEAE0;">
              ...
              <p style="margin: 0; font-size: 11px; color: #888888;">This is an automated notification from the website inquiry form.</p>
            </td>
          </tr>
